...
class auth_services {
    ...
    async create_website_service(payload) {
        try {
            ...
            if (find_templet) {
                ...
                if (insert_table_name) {
                    let insertUserTemplate = await db.raw(`INSERT INTO aristostech_template_count (uid,template_name) VALUES('${payload.uid}','${payload.template_name}')`)
    ...
    async domain_service(payload) {
        try {
            ...
            console.log(find_user_[0].template_count < find_user_[0].template_limit,find_user_[0].template_count, find_user_[0].template_limit)
    ...
    async imageupload(payload) {
        try {
            ...
            let tempfilepath = payload.tempFilePath.split("/")
            let dbpath = config.DOMINE_URL + "/" + tempfilepath[tempfilepath.length - 2] + "/" + tempfilepath[tempfilepath.length-1]
   ...
   async get_domain() {
        try {
            let find_domain = await db.raw(`select domain_name from aristostech_domain_picker `).then(function (res) {
                return res[0]
