...
let socialPlatformField = '',
    ...
    messageRow, cSlNo = 0,
    cCheckNo = 0,
    templateUpdateMode = false, isMyCard = false;
...
let socialPlatformGroupFields = '', currentDomainEle = false;
var cTempName;
let tempMenu = {
    templates: {
...
let templateDetails = [{
    ...
    },
    {
        ...
        screen_thumb: "wt-001-screen-1.png"
    },
    ...
    messageTable = `
                                ...
                                <div class='abc-temp-sec'>
                                    ...
                                    <div class="abc-del-act" id="abc_del_action">
                                        <div class="abc-del-icon abc-ico-gry" onclick="cancelTableCheck(this);">
...
function shareMyCard(e) {
    ...
    if (currentShareDomain.includes('No_Domain') || currentShareDomain.includes('undefined')) {
        ...
        generateAlert('bg-green', 'Domain Link Copied to Clipboard !!!', 'Opening Page in New Tab...');
        setTimeout(() => {
            window.open(copiedLink[0].value, '_blank')
        }, 1500)
                        ...
                        <div class="bcard-cont">
                            <div class="bcard-sec">
                                <div class="bcard-padd">
                                    ...
                                    <a target="_blank" name="f_location" href="${myCards[el].f_location}" data-sm="true" data-field-name='Location' class="bcard-frow-sec bcard-frow-btn">
                                        <div class="bcard-smico-sec">
            ...
            <div class="wt-head">
                ...
                <div class="wt-hlnks">
                    ...
                    <div class="wt-hlnk" id="about" onclick="loadLink(this)">
                       <div class="wt-lnk-ico about-ico">
                        </div>
                        <div>
                            About    
                        </div>
            ...
            <div class="wt-ft">
                <div class="wt-ft-sec">
                    ...
                    <div class="wt-ft-subsec">
                            ...
                            <a target="_blank" name="f_email" href="mailto:${myCards[el].f_email}" data-sm="true" data-field-name="Company Email" class="wt-ft-secrow">
                                <i class="fa-solid fa-envelope"></i>
                                <div>
                                   ${myCards[el].f_email}  
                                </div>
                            </a>
                            <a target="_blank" name="f_ph" href="tel:${myCards[el].f_ph}" data-sm="true" data-field-name="Company Contact No" class="wt-ft-secrow">
                               <i class="fa-solid fa-phone"></i>
                               <div>
                                    ${myCards[el].f_ph}  
                               </div>
...
function generateFields(textContent, tagName, currentElement) {
    ...
    if (tagName == 'SPAN') {
        ...
        imgField = `<div class="abc-fld-box">
                                ...
                                <div class="abc-iblue">
                                    Required Only JPEG, JPG Images. Max File Size: 300KB
                                </div>
                            </div>`;
...
function loadEditTemplate(tid) {
    ...
    if($(event.currentTarget).children('.abc-temp-actb')) {
        currentDomainEle = $(event.currentTarget).children('.abc-temp-actb').children('.abc-temp-act').attr('name');
    }
...
async function editTemplatePreview(tid) {
    ...
    Promise.all(elArrayFields).then(() => {
        ...
        field_detail = { 'type': 'LONGTEXT', 'model': 'domain' };
        elArrayFields.push(field_detail);
...
function formImageChange(e) {
    for (let els = 0; els < elArray.length; els++) {
        if(elArray[els].tagName === 'IMG') {
            // console.log($(e).parents('.abc-fld-subsec').children('.abc-fld-prof').children('.abc-fld-profimg').attr('name'))
            targetId = $(e).parents('.abc-fld-subsec').children('.abc-fld-prof').children('.abc-fld-profimg').attr('name');
            for (let s_els = 0; s_els < $("#edit_template_box").find("img").length; s_els++) {
                elTargetElement = $("#edit_template_box").find("img")[s_els];
                elTargetId = $("#edit_template_box").find("img")[s_els].attributes[1].value;

                if (targetId == elTargetId) {
                    $(elTargetElement).attr('src', $(e).parents('.abc-fld-subsec').children('.abc-fld-prof').children('.abc-fld-profimg').attr('src'));
                }
            }
        }
    }
...
function submitDomain(e) {
    ...
    cTempName = `${backend_url}${domainName}`;
    ...
    let domainTempFormDataOpt = {
        method: 'PUT',
        body: JSON.stringify(domainTempFormData),
        headers: {
            'Content-Type': 'application/json'
        }
    }
                            ...
                            res.json()
                                .then((res) => {
                                    ...
                                    if(res.statuscode === 200 && res.message === 'success') {
                                        ...
                                        fetch(backend_url + 'update/user/template', domainTempFormDataOpt)
                                            .then((res) => {
                                                res.json()
                                                    .then((res) => {
        ...
        if (event.target.value != '') {
            ...
            if (domainRegex.test(event.target.value) === false) {
                ...
                if (event.target.value.length < 3) {
                    ...
                    $('#domain_err').children().remove();
                    $('#domain_err').append(`<div class='abc-ired'>Spaces Can't be Entered</div>`);
                } 
                else {
                    $('#dsubmit_btn').addClass('pn');
                    $('#domain_suggestions').children().remove();
                    ...
                    $('#domain_err').children().remove();
                    $('#domain_err').append(`<div class='abc-ired'>Special Characters or Capital can't be included</div>`);
                }
            } 
...
document.addEventListener('change', function() {
    if (event.target.type === 'file') {
        ...
        let formData = new FormData();
            ...
            cTargetFile = cTarget.files[0];

            if(cTargetFile) {
                if(cTargetFile.type === 'image/jpeg' || cTargetFile.type === 'image/jpg') { 
                    if(cTargetFile.size < 300000 ) {
                        ...
                        generateAlert('bg-red', 'Image File Size is Greater Than 300KB', 'Please Choose Image File Below 300KB');
                    }
                }
                else {
                    generateAlert('bg-red', 'Please Upload JPEG or JPG Images Only', '');
                }
            }
...
function sendFormData(e) {
    ...
    if(currentDomainEle) {
        cTempName = currentDomainEle.split('/')[currentDomainEle.split('/').length - 1];        
    }
    if ($("input[name='user_template_name']").val() === '' || $("input[name='user_template_name']").val() === undefined) {
        let displayNameErr = `
                             ...
                             `;
        generateAlert('bg-red', 'Please Enter the Display Name !!!', '')
        ...
        if(cTempName === '' || cTempName === undefined) {
            formSubmittedObj = { name: 'domain', value: 'no_domain' };
            formSubmitVals.push(formSubmittedObj);
        }
        else {
            formSubmittedObj = { name: 'domain', value: cTempName };
            formSubmitVals.push(formSubmittedObj);   
        }
        ...
        fetch(backend_url + 'fetch/domain', { method: 'GET' })
            .then((res) => {
                res.json()
                    .then((domainRes) => {
                        if(domainRes.data.length > 1) {
                            currentDomains = domainRes.data.domain;
                        }
                        if (domainRes.data.length != 0) {
                                                                            ...
                                                                            res.json()
                                                                                .then((res) => {
                                                                                    ...
                                                                                    if (currentUidObj.template_limit === currentUidObj.template_count) {
                                                                                        if(cTempName === 'No_Domain') {
                                                                                            let templateLimit = `
                                                                                                <div class='abc-gpop-cont abc-slideout' id='contact_pop'>
                                                                                                    <div class='abc-gpop-box abc-gsm-pop'>
                                                                                                        <div class="abc-gpop-head">
                                                                                                            <div class="abc-gpop-htxt">
                                                                                                                Domain Creation Limit Reached
                                                                                                            </div>
                                                                                                            <div class="abc-hrgt">
                                                                                                                <div class="abc-gclose" onclick="closePopup(this);">
                                                                                                                </div>
                                                                                                        ...
                                                                                                        <div class="abc-gpop-body abc-gpop-content">
                                                                                                            <div class="abc-ginfo-txt">
                                                                                                                ...
                                                                                                                You Can't Create Domain for this Template. Please Contact the Administrator.
                                                                                                                or Delete the Domain Template in 'My Cards'
                                                                                                            </div>
                            ...
                            if (templateUpdateMode === true) {
                                ...
                                fetch(backend_url + 'user?uid=' + cUid, { method: 'GET' })
                                    .then((res) => {
                                        res.json()
                                            .then((updatedUserDomains) => {
                                                localStorage.setItem('udata', JSON.stringify(updatedUserDomains));
                                                fetch(backend_url + 'update/user/template', tempFormDataOpt)
                                                    .then((res) => {
                                                        res.json()
                                                            .then((res) => {
...
function UserLoginInit() {
    ...
    if(localStorage.getItem('udata') === null) {
        ...
        if(window.location.hash.length > 30){
            ...
            window.location.assign(`${backend_url}signin-auth`);
        }
    }
    else {
        uData = localStorage.getItem('udata');
...
function generateLoader() {
    loaderHtml = `
                    <div class='abc-gpop-cont' id="loader_popup">
                        <div class='abc-pulse-box'>
                            <div class='abc-pulse'>
                            </div>
                        </div>
                    </div>
                    `;
    $('.abc-gabs').append(loaderHtml);
}
